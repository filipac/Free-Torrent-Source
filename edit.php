...
if (!isset($CURUSER) || ($CURUSER["id"] != $row["owner"] && !ur::ismod())) {
	...
	print("<form method=post name=edittorrent action=takeedit.php enctype=multipart/form-data>\n");
...
if ((strpos($row["ori_descr"], "<") === false) || (strpos($row["ori_descr"], "&lt;") !== false))
  ...
  $c = " checked";
  print("<tr><td class=\"heading\" valign=\"top\" align=\"right\">Description</td><td valign=\"top\" align=left>");
  textbbcode("edittorrent","descr",($row["ori_descr"]));
  print ("</td></tr>\n");
	//tr("Description", "<textarea name=\"descr\" rows=\"10\" cols=\"80\">" . htmlspecialchars() . "</textarea><br>(HTML is not allowed. <a href=page.php?type=tags>Click here</a> for information on available tags.)", 1);
	...
	if(ur::ismod()) {
		...
		tr("Double upload", "<input type='checkbox' name='doubleupload'" . (($row["doubleupload"] == "yes") ? " checked='checked'" : "" ) . " value='1' /> Double Upload (upload stats are recorded twice)", 1);
	...
	print("<tr><td colspan=\"2\" align=\"center\"><input class=button type=\"submit\" value='Edit it!' style='height: 25px; width: 100px'> <input type=reset value='Revert changes' style='height: 25px; width: 100px'></td></tr>\n");
	print("</table>\n");
	print("</form>\n");
	if(ur::ismod() || ($usergroups['candeletetorrent'] == 'yes' && $CURUSER['id'] == $row['owner'])) {
	print("<p>\n");
