...
print("<table border=1 width=100% cellspacing=0 cellpadding=5><tr><td class=colhead align=left>".
"Please search torrents before adding an Offer!</td></tr><tr><td align=left class=clearalt6><form method=get action=browse.php>".
"<input type=text name=search size=40 value=\"".htmlspecialchars($searchstr)."\" />in <select name=cat> <option value=0>(all types)</option>");
...
print(" ".$catdropdown." </select> ".$deadchkbox." <input type=submit value=Search! class=btn /></form></td></tr></table><br>\n");
...
"<tr><td align=right class=clearalt6></td><td align=left class=clearalt6>\n");
textbbcode("compose","body","$body");
print("</td></tr><tr><td align=center colspan=2 class=clearalt6><input type=submit class=btn value=\"Add Offer!\" /></td></tr></table></form><br><br>\n");

//===list other offers
$res = mysql_query("SELECT users.username, offers.id, offers.userid, offers.name, offers.added, uploaded, downloaded, categories.image, categories.name as cat FROM offers inner join categories on offers.category = categories.id inner join users on offers.userid = users.id order by offers.id desc LIMIT 10") or sqlerr(__FILE__,__LINE__);
$num = mysql_num_rows($res);

print("<table border=1 width=100% cellspacing=0 cellpadding=5><tr><td class=colhead align=left width=50>Category</td>".
...
"<input class=button type=\"submit\" value=\"Show All\" class=btn /></form></td></tr></table>\n");
stdfoot();
die;
}
//=== end add offer
...
begin_frame("Details for Offer:$s",true,'10','100%'); 
...
"<input class=btn type=submit value=Allow></td></form><td align=center class=clearalt6><form method=post action=viewoffers.php?id=$id&finish_offer=1>".
"<input type=hidden value=$id name=finish><input class=btn type=submit value=\"Let votes decide\"></form></td></tr></table></td></tr>");
...
"<td align=left class=clearalt6><form action=report.php?reportofferid=$id method=\"post\">".
" for breaking the rules <input class=btn type=submit name=submit value=\"Report Offer\"></form>".
...
if($yes == '0' && $no == '0')
stderr("Sorry", "No votes yet... <a class=altlink href=viewoffers.php?id=$offid&off_details=1>Back to Offer details</a>",false);
...
"<table border=1 width=100% cellspacing=0 cellpadding=5><tr><td class=colhead align=left colspan=2><h1>Edit Offer ".
...
textbbcode("compose","body","$body");
print("</td></tr><tr><td align=center  class=clearalt6 colspan=2><input type=submit value='Edit Offer' class=btn></td></tr></form><br><br></table><br>\n"); 
...
if ($voted)
stderr("You've already voted", "<p>You've already voted, max 1 vote per offer</p><p>Back to the <a class=altlink href=viewoffers.php?id=$offerid&off_details=1><b>offer details</b></a></p>",false);
else 
{
...
if ($sure == 0)	 
stderr("Delete Offer", "You`re about to delete this offer. Click\n <a class=altlink href=viewoffers.php?id=$offer&del_offer=1&sure=1>here</a>, if you`re sure.",false);
elseif ($sure == 1){ 
...
print("<div align=center><table border=1 width=100% cellspacing=0 cellpadding=5><tr><td class=colhead align=center><font size=4>".
...
"</form><br><form method=get action=viewoffers.php><b>Search offers: </b><input type=text id=specialboxg name=search>".
"<input type=submit align=center value=Search class=btn></form></td></tr></table><br /><br>");
...
if (!$num)
	echo '<tr><td colspan=7 align=center><b>Nothing Found!</b></td></tr>';
for ($i = 0; $i < $num; ++$i)
{	 
//=======change colors
		if($count2 == 0)
