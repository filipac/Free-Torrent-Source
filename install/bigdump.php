...
$db_server   = $DATABASE['mysql_host']; 
$db_name     = $DATABASE['mysql_db'];
$db_username = $DATABASE['mysql_user']; 
$db_password = $DATABASE['mysql_pass']; 
// Other Settings
$filename        = 'torrent.sql';     // Specify the dump filename to suppress the file selection dialog
    ...
    { if ($dirfile != "." && $dirfile != ".." && $dirfile!=basename($_SERVER["SCRIPT_FILENAME"]))
      { if (!$dirhead)
        ...
        if (!eregi("\.gz$",$dirfile) || function_exists("gzopen")) 
          echo ("<td><a href=\"".$_SERVER["PHP_SELF"]."?action=step4&start=1&fn=$dirfile&foffset=0&totalqueries=0\">Start Import</a> into $db_name at $db_server</td>\n");
        else
          echo ("<td>&nbsp;</td>\n");
        echo ("<td><a href=\"".$_SERVER["PHP_SELF"]."?action=step4&delete=$dirfile\">Delete file</a></td></tr>\n");
      } 
...
?>
<script>
window.location = "<?=$_SERVER["PHP_SELF"]?>?action=step4&start=1&fn=<?=$filename?>&foffset=0&totalqueries=0";
</script>
<?php
}
// File Upload Form
...
?>
<form method="POST" action="<?php echo ($_SERVER["PHP_SELF"]); ?>?action=step4" enctype="multipart/form-data">
...
// Finish message and restart the script
  ...
  if (!$error)
  {
    ...
    { if ($delaypersession!=0)
      ...
      echo ("<script language=\"JavaScript\" type=\"text/javascript\">window.setTimeout('location.href=\"".$_SERVER["PHP_SELF"]."?action=step4&start=$linenumber&fn=".$_REQUEST["fn"]."&foffset=$foffset&totalqueries=$totalqueries\";',500+$delaypersession);</script>\n");
      echo ("<noscript>\n");
      echo ("<p><a href=\"".$_SERVER["PHP_SELF"]."?action=step4&start=$linenumber&fn=".$_REQUEST["fn"]."&foffset=$foffset&totalqueries=$totalqueries\">Continue from the line $linenumber</a> (Enable JavaScript to do it automatically)</p>\n");
      echo ("</noscript>\n");
      echo ("<p>Press <a href=\"".$_SERVER["PHP_SELF"]."?action=step4\">STOP</a> to abort the import <b>OR WAIT!</b></p>\n");
    }
  }
  else 
...
if ($error)
  echo ("<p><a href=\"".$_SERVER["PHP_SELF"]."?action=step4\">Start from the beginning</a> (DROP the old tables before restarting)</p>\n");
