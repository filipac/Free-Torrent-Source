...
if ($action == "edit")
{
	...
	if (mysql_num_rows($res) == 0)
		stderr("Error","No poll found with this ID");
...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
  ...
  if ($pollid)
    ...
    {
    	global $pollfid;
        $forum_id = $pollfid; // set the id of the forum, where do you want topic to be posted
        $time = get_date_time();
        $body = "$question\n\n";
        ...
        $body .= '[color=Red][font=Comic Sans MS][size=1]Note: This message was created automaticly.[/size][/font][/color]';
        
        mysql_query("INSERT INTO topics (userid, subject, forumid) VALUES ('$CURUSER[id]', '$question', '$forum_id')");
        $topic_id = mysql_result(mysql_query("SELECT id FROM topics WHERE subject = '$question' AND userid = '$CURUSER[id]' AND forumid = '$forum_id'"), 0);
        mysql_query("INSERT INTO posts (topicid, userid, added, body) VALUES ('$topic_id', '$CURUSER[id]', '$time', '$body')");
        $post_id = mysql_result(mysql_query("SELECT id FROM posts WHERE topicid = '$topic_id' AND userid = '$CURUSER[id]' AND added = '$time'"), 0);
        mysql_query("UPDATE topics SET lastpost=$post_id WHERE id=$topic_id");
    }
    }
    }
...
global $pollf;
  	if($pollf == 'yes')
tr("Make an thread in forum for this poll","<input type=\"checkbox\" name=\"epoll\" value=\"yes\">",1);
?>
