...
// ===================================
begin_frame("Delete Reports", true, '10', '100%');
begin_table(1);
...
print("<tr><td class=colhead align=center>Added</td><td class=colhead align=center>Reported by</td><td class=colhead align=center>Reporting</td><td class=colhead align=center>Type</td><td class=colhead align=center>Reason</td><td class=colhead align=center>Dealt With</td><td class=colhead align=center>Del</td></tr>");
...
if ($arr[type] == "forum")
{
print("<tr><td>$arr[added]</td><td><a href=userdetails.php?id=$arr[addedby]><b>$arr[username]</b></a></td><td align=left><a href=$type.php?action=viewtopic&topicid=$arr[votedfor]&page=p#$arr[votedfor_xtra]><b>$subject</b></a></td><td align=left>$arr[type]</td><td align=left>$arr[reason]</td><td align=left>$dealtwith</td><td><input type=\"checkbox\" name=\"delreports[]\" value=\"" . $arr[id] . "\" /></td></tr>\n");
}
else if ($arr[type] == "comment")
{
print("<tr><td>$arr[added]</td><td><a href=userdetails.php?id=$arr[addedby]><b>$arr[username]</b></a></td><td align=left><a href=$type.php?id=$torrent&viewcomm=$arr[votedfor]#comm$arr[votedfor]><b>$comment_username</b></a></td><td align=left>$arr[type]</td><td align=left>$arr[reason]</td><td align=left>$dealtwith</td><td><input type=\"checkbox\" name=\"delreports[]\" value=\"" . $arr[id] . "\" /></td></tr>\n");
}
else {
print("<tr><td>$arr[added]</td><td><a href=userdetails.php?id=$arr[addedby]><b>$arr[username]</b></a></td><td align=left><a href=$type.php?id=$arr[votedfor]><b>$name</b></a></td><td align=left>$arr[type]</td><td align=left>$arr[reason]</td><td align=left>$dealtwith</td><td><input type=\"checkbox\" name=\"delreports[]\" value=\"" . $arr[id] . "\" /></td></tr>\n");
}
}
print("<p align=center><input type=button value=\"Check All\" onClick=\"this.value=check(form)\"><input type=submit value=\"Delete selected\"></p>");
print("</form>");


print("<tr><td align=center colspan=7><form method=\"get\" action=\"reports.php#add\"><input type=\"submit\" value=\"Return\" style='height: 18px' /></form></td></tr>\n");
end_table();
end_frame();
end_main_frame();
stdfoot();
