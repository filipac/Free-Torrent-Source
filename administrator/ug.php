...
$FTSpass['copy'] = true;
$rootpath = '../' ;
include_once $rootpath . 'include/bittorrent.php' ;
loggedinorreturn() ;
if ( ! ur::isadmin() )
{
...
echo '<link rel="stylesheet" type="text/css" href="controlpanel.css" /><body>';
...
$gid = !empty($_GET['id']) ? $_GET['id'] : '0';
	...
	if (mysql_num_rows($query) == 0) {
		...
		$add_args1 = parse_args(unserialize($usergroup['args']));
   		$usergroup += $add_args1;
...
yesno('Has antiflood check?', 'antifloodcheck', $usergroup['antifloodcheck'] == 'yes' ? 'yes' : 'no');
inputbox('Antiflood time(in seconds)','antifloodtime',$usergroup['antifloodtime'],10,'<BR>The interval that must wait between shouts,forum posts,torrent comments etc...');
echo'</tbody>';
...
yesno('Can post in forum topics', 'arg[canpostintopics]', $usergroup['canpostintopics'] == 'yes' ? 'yes' : 'no');
...
$updateset[] = 'antifloodcheck = '.sqlesc($antifloodcheck);
$updateset[] = 'antifloodtime = '.sqlesc($antifloodtime);
$updateset[] = 'args = '.sqlesc(serialize($arg));
	
	mysql_query('UPDATE usergroups SET  ' . implode(", ", $updateset) . ' WHERE id='.sqlesc($gid)) or sqlerr(__FILE__, __LINE__);
	redirect('administrator/ug.php', 'The usergroup has successfully been updated.');
}elseif($action== 'deleteug') {
$todel = $_GET['todel'];
$h = sql_query("DELETE FROM usergroups WHERE id = '$todel'") or die(mysql_error());
if($h)
redirect('administrator/ug.php','Usergroup Deleted','OK');
die;
}elseif($action == 'newgr') {
		#stdhead('Usergroups '.UG_VERSION.' - Edit UserGroups -');
$gid = !empty($_GET['id']) ? $_GET['id'] : '0';
...
yesno('Has antiflood check?', 'antifloodcheck', 'no');
inputbox('Antiflood time(in seconds)','antifloodtime','60',10,'<BR>The interval that must wait between shouts,forum posts,torrent comments etc...');
echo'</tbody>';
...
yesno('Can post in forum topics', 'arg[canpostintopics]');
	echo '</tbody><tbody><tr><td class="colhead" colspan="2" align="center">Permissions: Viewing</td></tr>';
